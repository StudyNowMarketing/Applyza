...
const renderContent = (content: string) => {
  ...
  return paragraphs.map((p, i) => {
    ...
    // Detect subheadings: short lines (<80 chars), capitalised start, no period at end
    const isSubheading =
      trimmed.length < 80 &&
      !trimmed.endsWith(".") &&
        ...
        <div className="container max-w-3xl">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink asChild><Link to="/blog" className="text-primary-foreground/60">Blog</Link></BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator className="text-primary-foreground/40" />
              <BreadcrumbItem><BreadcrumbPage className="text-primary-foreground/80 line-clamp-1">{post.title}</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
      </section>

      <article className="bg-background py-12 md:py-16 flex-1">
        <div className="container max-w-[720px]">
          {post.category && (
            <span className="inline-block border border-secondary text-secondary text-xs font-medium px-3 py-1 rounded-full mb-4">
              {post.category}
            </span>
          )}
          <h1 className="text-3xl md:text-4xl font-extrabold text-primary mb-4 leading-tight">{post.title}</h1>
          <p className="text-sm text-muted-foreground mb-10">
            {post.author}{post.published_at && ` · ${format(new Date(post.published_at), "MMMM d, yyyy")}`}
          </p>
          ...
          <div className="border-t mt-12 pt-8">
            ...
            <div className="flex gap-3">
              ...
              <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer"
                ...
                <svg viewBox="0 0 24 24" className="w-4 h-4 fill-primary"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              </a>
          ...
          {related && related.length > 0 && (
            <div className="mt-12">
              <h3 className="text-xl font-bold text-primary mb-6">Related Posts</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {related.map((r) => (
                  <Link key={r.id} to={`/blog/${r.slug}`} className="group bg-card rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all hover:-translate-y-1">
                    <div className="h-32 bg-gradient-to-br from-accent/20 via-secondary/15 to-primary/10" />
                    <div className="p-4">
          ...
          <div className="mt-12 bg-accent/10 rounded-2xl p-8 text-center">
            <h3 className="text-lg font-bold text-primary mb-2">Need personalised guidance?</h3>
            <p className="text-sm text-muted-foreground mb-4">Our expert counsellors are here to help — completely free.</p>
