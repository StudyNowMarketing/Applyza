  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div />
        <Button variant="teal" className="rounded-full" asChild><Link to="/admin/events/new"><Plus size={16} className="mr-1" /> Add New Event</Link></Button>
      </div>
              ...
              : rows && rows.length > 0 ? rows.map((r) => (
                <tr key={r.id} className="border-b hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3"><div className="flex gap-1">
                    <Button variant="ghost" size="icon" className="h-7 w-7" asChild><Link to={`/admin/events/edit/${r.id}`}><Pencil size={14} /></Link></Button>
